...
function helloTriangle() {
  ...
  const triangleShaderProgram = gl.createProgram() as WebGLProgram;
  ...
  const shapeLocationUnifrom = gl.getUniformLocation(triangleShaderProgram, "shapeLocation");
  const shapeSizeUniform = gl.getUniformLocation(triangleShaderProgram, "shapeSize");
  const canvasSizeUnifrom = gl.getUniformLocation(triangleShaderProgram, "canvasSize");
  if (!shapeLocationUnifrom || !shapeSizeUniform || !canvasSizeUnifrom) {
    showError(
      `FAILED to get the uniform locations (shapeLocation=${shapeLocationUnifrom})` +
        `, (shapeSize=${shapeSizeUniform}), (canvasSize=${canvasSizeUnifrom})`
    );
    return;
  }
  ...
  gl.uniform2f(shapeSizeUniform, canvas.width, canvas.height);

  gl.uniform1f(shapeSizeUniform, 200);
  gl.uniform2f(shapeLocationUnifrom, 300, 600);
  gl.drawArrays(gl.TRIANGLES, 0, 3);

  gl.uniform1f(shapeSizeUniform, 100);
  gl.uniform2f(shapeLocationUnifrom, 650, 300);
  gl.drawArrays(gl.TRIANGLES, 0, 3);
}
